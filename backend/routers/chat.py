...
def _expand_company_policy_keywords(query: str) -> str:
    """사내규정 관련 키워드 확장"""
    
    # 사내규정 관련 키워드 매핑
    keyword_expansions = {
        "휴가": "휴가 연차 유급휴가 병가 경조사휴가 출산휴가 육아휴가",
        "급여": "급여 임금 봉급 월급 연봉 수당 보너스 인센티브",
        "근무": "근무 출근 퇴근 근무시간 업무시간 야근 초과근무",
        "복리후생": "복리후생 복지 혜택 지원금 보험 건강검진",
        "인사": "인사 승진 전보 발령 평가 고과 채용 퇴직",
        "교육": "교육 연수 교육훈련 세미나 워크샵 스킬업",
        "출장": "출장 외근 파견 업무여행 교통비 숙박비",
        "보안": "보안 정보보안 개인정보 기밀 보안규정",
        "징계": "징계 처벌 제재 경고 견책 정직 해고",
        "안전": "안전 산업안전 작업안전 안전교육 사고예방"
    }
...
def _rerank_policy_results(search_results: List[Dict], original_question: str) -> List[Dict]:
    """사내규정 관련성 기준으로 검색 결과 재정렬"""
    
    # 사내규정 관련 우선순위 키워드
    high_priority_keywords = [
        "규정", "규칙", "지침", "기준", "절차", "방법", "조건", "요건",
        "휴가", "급여", "근무", "출근", "퇴근", "인사", "복리후생", "교육",
        "출장", "보안", "징계", "안전", "평가", "승진", "채용", "퇴직"
    ]
    
    medium_priority_keywords = [
        "신청", "승인", "처리", "관리", "운영", "시행", "적용", "대상",
        "기간", "일수", "시간", "비용", "지원", "혜택", "보상", "제재"
    ]
    
    def calculate_policy_relevance_score(result: Dict) -> float:
        """사내규정 관련성 점수 계산"""
        ...
        if any(keyword in file_name for keyword in ["규정", "지침", "기준", "정책", "policy"]):
            relevance_score += 0.1
        
        # 시트명 기반 가중치
        sheet_name = metadata.get("sheet_name", "").lower()
        if any(keyword in sheet_name for keyword in ["규정", "지침", "기준", "정책"]):
            relevance_score += 0.05
        
        # 텍스트 내용 기반 가중치
        high_priority_count = sum(1 for keyword in high_priority_keywords if keyword in text)
        medium_priority_count = sum(1 for keyword in medium_priority_keywords if keyword in text)
        
        relevance_score += high_priority_count * 0.02
        relevance_score += medium_priority_count * 0.01
        
        # 질문과의 키워드 매칭 가중치
        question_keywords = original_question.lower().split()
        matching_keywords = sum(1 for keyword in question_keywords if keyword in text)
        relevance_score += matching_keywords * 0.01
        
        return min(relevance_score, 1.0)  # 최대 1.0으로 제한
    
    # 관련성 점수 계산 및 정렬
    for result in search_results:
        result["policy_relevance_score"] = calculate_policy_relevance_score(result)
    
    # 관련성 점수 기준으로 정렬
    return sorted(search_results, key=lambda x: x["policy_relevance_score"], reverse=True)
